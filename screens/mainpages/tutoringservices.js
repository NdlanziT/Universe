...
const closeImageModal = () => {
    ...
    try {
      ...
      setPostComments(prevComments => 
        prevComments.map(prevComment => 
            prevComment.id === currentcommentid 
                ? {
                    ...prevComment, 
                    content: comment, 
                    createdat: postdate(),
  ...
  const scrollViewRef = useRef(null);
    ...
    async function fetchFollowingsPosts(currentUserId,myfollowing) {
      try {
        // Set up query to get only posts with category "market place"
        const postsCollectionRef = collection(db, "post");
        const marketplaceQuery = query(postsCollectionRef, where("category", "==", "tutoring service"));
...
// Close edit modal
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [following]); // Add 'following' dependency to re-fetch when the following list changes
  ...
  useEffect(() =>{
      ...
      setPost(prevPosts => {
          ...
          return [...prevPosts, ...filteredNewPosts]; // Combine existing and new posts
      });
  });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.topbar}>
        <Text style={styles.pageTitle}>Tutoring Service</Text>
      </View>
      {/* <TextInput
